...
import { getUserFromToken, validateToken } from "../middlewares/middlewares.js";
import Song from "../models/song.model.js";
import PDFDocument from "pdfkit";
import path from "path";
import { fileURLToPath } from "url";
import mongoose from "mongoose";
import { transposeChord, chordRegex } from "../config/index.js";
...
const getAllPlaylists = async (req, res) => {
  ...
  let query = {
    isPublic: true,
    name: { $regex: String(searchRegex), $options: "i" },
  };
  if (search === "undefined") {
...
const getPlaylist = async (req, res) => {
  try {
    ...
    const playlist = await Playlist.findById({ _id: id, isPublic: true })
      .populate("user")
      .populate({ path: "songs", populate: { path: "user", model: "User" } });
    if (!playlist)
      return res
        .status(404)
    ...
    return res
      .status(403)
      .json({ success: false, message: "Unauthorized to view this playlist" });
  } catch (err) {
    mongooseErrors(err, res);
  }
};
...
const getMyPlaylists = async (req, res) => {
  try {
    const response = await Playlist.find({ user: req.user._id })
      .populate("user")
      .populate("songs")
      .sort({ likes: -1, _id: 1 });
...
const getFavouritePlaylists = async (req, res) => {
  try {
    ...
    const playlists = await Playlist.find({
      _id: { $in: response.favouritePlaylists },
    })
      .populate("user")
      .sort({ likes: -1, _id: 1 });
...
const likePlaylist = async (req, res) => {
  try {
    ...
    if (user.favouritePlaylists.includes(req.params.id))
      return res
        .status(400)
        .json({ success: false, message: "Playlist already liked" });
    const response = await User.findOneAndUpdate(
      { _id: req.user._id },
      { $push: { favouritePlaylists: req.params.id } },
      { new: true }
    ).populate("favouritePlaylists");
    ...
    res.json({
      success: true,
      favouritePlaylists: response.favouritePlaylists,
    });
  } catch (err) {
    mongooseErrors(err, res);
  }
...
const unlikePlaylist = async (req, res) => {
  try {
    ...
    if (!user.favouritePlaylists.includes(req.params.id))
      return res
        .status(400)
        .json({ success: false, message: "Playlist not liked" });
    const response = await User.findOneAndUpdate(
      { _id: req.user._id },
      { $pull: { favouritePlaylists: req.params.id } },
      { new: true }
    ).populate("favouritePlaylists");
    await Playlist.findOneAndUpdate(
      { _id: req.params.id },
    ...
    if (req.user.role === "admin") {
      ...
      if (playlist.user.toString() === req.user._id.toString()) {
        ...
        await User.updateMany(
          { favouritePlaylists: { $in: [req.params.id] } },
          { $pull: { favouritePlaylists: req.params.id } }
        );
        return res.json({
          success: true,
  ...
  try {
    ...
    if (!playlist)
      return res
        ...
        .json({ success: false, message: "Playlist not found" });
    if (
      playlist.user._id.toString() !== req.user._id &&
      req.user.role !== "admin"
    )
      return res
        .status(403)
...
const addSongToPlaylists = async (req, res) => {
  ...
  try {
    await Playlist.updateMany(
      { _id: { $in: playlistIds }, user: userId, songs: { $nin: [songId] } },
      { $push: { songs: songId } }
    );
    await Song.updateOne(
      { _id: songId },
      { $inc: { addedToPlaylist: playlistIds.length } }
    );
...
const removeFromPlaylist = async (req, res) => {
  ...
  if (!songId || !playlistId)
    return res
      .status(400)
      .json({ success: false, message: "Missing songId or playlistId" });
  try {
    ...
    await Playlist.updateOne(
      { _id: playlistId, songs: { $in: [songId] } },
      { $pull: { songs: songId } }
    );
...
const downloadPlaylist = async (req, res) => {
  try {
    ...
    playlist.songs.forEach((song, index) => {
      ...
      doc.fontSize(8);
      let step = 0;
      if (user && user.songSteps.find((s) => s.songId.toString() === song._id.toString())) step = user.songSteps.find((s) => s.songId.toString() === song._id.toString()).step;
      if (step !== 0 && step !== null) {
        song.text = song.text.replace(chordRegex, (match) => transposeChord(match, step));
      }
      doc.text(`Step: ${step}`, { align: "center" });
      doc.moveDown(2);
      doc.text(song.text);
      doc.moveDown(2);
